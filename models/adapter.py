...
#Adapter结构,两个全连接层,一次激活函数,默认带有残差连接
class Adapter(nn.Module):
    ...
    def forward(self, x,type=1):
        ...
        if type:
            BT, C, H, W = x.shape
            x = x.view(BT, C, -1)  # 将 (BT, C, H, W) 转换为 (BT, C, H*W)
            x = x.permute(0, 2, 1)  # 将 (BT, C, H*W) 转换为 (BT, H*W, C)
            xs = self.D_fc1(x)
            xs = self.act(xs)
            xs = self.D_fc2(xs)
            if self.skip_connect:
                ...
                x = xs
            x = x.permute(0, 2, 1)  # 将 (BT, H*W, C) 转换为 (BT, C, H*W)
            x = x.view(BT, C, H, W)  # 将 (BT, C, H*W) 转换为 (BT, C, H, W)
            return x
        else:
            x = x.permute(1,0,2)
            xs = self.D_fc1(x)
