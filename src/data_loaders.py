...
class BaseDatasetIter(Dataset):
    ...
    def is_valid_text(self,text):
        # 正则表达式匹配只包含数字0-9、小写字母a-z和大写字母A-Z的字符串
    ...
    def __getitem__(self, index):
        ...
        # TODO 去除错误数据
        try:
            anns = [ann for ann in anns if Polygon(ann['poly']).buffer(0).is_valid and self.is_valid_text(ann['text'])]
        except TypeError as e:
            print(anns)
        # TODO 加载gt_box
        gt_box = [ann['poly'] for ann in anns]
        # TODO 转换字符串
        str_data = ()
        ...
        text, length = self.convert.encode_batch(str_data)
        # TODO 加载目标数据
        img_origin = cv2.imread(image_path,cv2.IMREAD_GRAYSCALE)
        img_origin = np.expand_dims(img_origin, axis=-1)
        img_origin = np.transpose(img_origin, (2, 0, 1))
...
class TotalTextDatasetIter(BaseDatasetIter):
    def __init__(self, train_dir, train_gt_dir, ignore_tags,convert, **kwargs):
        super().__init__(train_dir, train_gt_dir, ignore_tags, convert,**kwargs)
    def load_metadata(self, img_dir, gt_dir):
        ...
        for img_fp in img_fps:
            ...
            gt_fn = "poly_gt_img{}.txt".format(img_id)
    ...
    def make_pair(self,data):
        ...
        for item in data:
            ...
            item = item.strip()
            # Extract key and value using regular expressions
            match = re.match(r'(\w+):\s*(.*)', item)
            if match:
                key, value = match.groups()
                # Convert string value to its actual type (list, int, etc.)
                try:
                    # Replace `u'` with `'` to handle unicode strings in Python 3
                    value = value.replace("u'", "'")
                    # Replace spaces with commas inside the double brackets
                    value = re.sub(r'\[\[([^\[\]]+)\]\]', lambda m: '[[' + ', '.join(m.group(1).split()) + ']]', value)
        ...
        x_values = np.array(list(map(float, x_values)))
        y_values = np.array(list(map(float, y_values)))
        # Combine x and y values into (x, y) pairs and reshape
        xy_pairs = np.vstack((x_values, y_values)).T.tolist()
        return np.array(xy_pairs)

    def extract_transcriptions_value(self,data):
        transcriptions_value = None
        for item in data:
            if 'transcriptions' in item:
                # 使用正则表达式匹配 transcriptions 中的值
                match = re.search(r"\[u'(.*?)'\]", item)
                if match:
                    transcriptions_value = match.group(1)
                    break
        return transcriptions_value
    def load_all_anns(self, gt_paths):
        ...
        for gt in gt_paths:
            ...
            for line in reader:
                ...
                poly = self.make_pair(line)
                label = self.extract_transcriptions_value(line)
                # TODO 没找到则跳过
                if label == None:
                    continue
                if len(poly) < 4:
...
@hydra.main(config_path="../myconfig.yaml", strict=False)
def run(cfg):
